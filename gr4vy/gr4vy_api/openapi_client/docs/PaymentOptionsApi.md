...
[**post_list_payment_options**](PaymentOptionsApi.md#post_list_payment_options) | **POST** /payment-options | List payment options
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    locale = "en-US" # str | An ISO 639-1 Language Code and optional ISO 3166 Country Code. This locale determines the language for the labels returned for every payment option. (optional) if omitted the server will use the default value of "en"
...
# **post_list_payment_options**
> PaymentOptions post_list_payment_options()

List payment options

Returns a list of available payment method options for the combination of amount, currency, country, metadata and list of cart items.  If the amount is zero, payment options which do not support zero amounts, will be omitted in the response.
...
with openapi_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = payment_options_api.PaymentOptionsApi(api_client)
    payment_options_request = PaymentOptionsRequest(
        amount=1299,
        locale="en-US",
        currency="USD",
        country="US",
    ...
    ) # PaymentOptionsRequest |  (optional)

    # example passing only required values which don't have defaults set
    # and optional values
    try:
        # List payment options
        api_response = api_instance.post_list_payment_options(payment_options_request=payment_options_request)
        pprint(api_response)
    except openapi_client.ApiException as e:
        print("Exception when calling PaymentOptionsApi->post_list_payment_options: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **payment_options_request** | [**PaymentOptionsRequest**](PaymentOptionsRequest.md)|  | [optional]
