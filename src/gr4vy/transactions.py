...
class Transactions(BaseSDK):
    ...
    def increment_authorization(
        ...
        *,
        ...
        timeout_ms: Optional[int] = None,
        http_headers: Optional[Mapping[str, str]] = None,
    ) -> models.TransactionAuthorizationIncrement:
        r"""Increment transaction authorization
        ...
        :param amount: The amount by which to increment the authorization, in the smallest currency unit of the transaction's currency. For example, `1299` cents to increment the authorization by `$12.99`.
        :param merchant_account_id: The ID of the merchant account to use for this request.
        :param idempotency_key: A unique key that identifies this request. Providing this header will make this an idempotent request. We recommend using V4 UUIDs, or another random string with enough entropy to avoid collisions.
        :param retries: Override the default retry configuration for this method
        :param server_url: Override the default server URL for this method
        ...
        request = models.IncrementTransactionAuthorizationRequest(
            transaction_id=transaction_id,
            merchant_account_id=merchant_account_id,
            idempotency_key=idempotency_key,
            transaction_authorization_increment_create=models.TransactionAuthorizationIncrementCreate(
                amount=amount,
            ),
        )
        ...
        req = self._build_request(
            method="POST",
            path="/transactions/{transaction_id}/authorization/increment",
            base_url=base_url,
            url_variables=url_variables,
            request=request,
            request_body_required=True,
            ...
            accept_header_value="application/json",
            http_headers=http_headers,
            _globals=models.IncrementTransactionAuthorizationGlobals(
                merchant_account_id=self.sdk_configuration.globals.merchant_account_id,
            ),
            security=self.sdk_configuration.security,
            get_serialized_body=lambda: utils.serialize_request_body(
                request.transaction_authorization_increment_create,
                False,
                False,
                "json",
                models.TransactionAuthorizationIncrementCreate,
            ),
            allow_empty_value=None,
            timeout_ms=timeout_ms,
        )
        ...
        http_res = self.do_request(
            hook_ctx=HookContext(
                config=self.sdk_configuration,
                base_url=base_url or "",
                operation_id="increment_transaction_authorization",
                oauth2_scopes=None,
                security_source=get_security_from_env(
                    self.sdk_configuration.security, models.Security
                ),
        ...
        if utils.match_response(http_res, "200", "application/json"):
            return unmarshal_json_response(
                models.TransactionAuthorizationIncrement, http_res
            )
        if utils.match_response(http_res, "400", "application/json"):
            response_data = unmarshal_json_response(errors.Error400Data, http_res)
            raise errors.Error400(response_data, http_res)
