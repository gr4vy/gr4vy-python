...
def test_add_shipping_details():
    ...
    assert client.add_buyer_shipping_details(
        buyer_id, **shipping_detail_request
    )
...
def test_create_new_payment_service():
    ...
    payment_service_id = client.create_new_payment_service(**payment_service_request)["id"]
...
def test_get_reportExections():
    report_execution_id = client.list_all_report_executions()["items"][0].get("id", None)
...
def test_capture_transaction():
    ...
    if transaction["status"] == "authorization_succeeded":
        ...
        capture = client.capture_transaction(transaction_id=transaction_id, **transaction_capture_request)
...
def test_refund_transaction():
    ...
    if transaction["status"] == "authorization_succeeded":
        ...
        transaction_capture_request = {"amount":1299}
        capture = client.capture_transaction(transaction.get('id'), **transaction_capture_request)
